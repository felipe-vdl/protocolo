...
const assuntoFormSchema = z.object({
  name: z.string(),
});
...
const AssuntoCreate = () => {
  ...
  const [form, setForm] =
    useState<z.infer<typeof assuntoFormSchema>>(formInitialState);

  const handleSubmit = async (evt: React.FormEvent<HTMLFormElement>) => {
    evt.preventDefault();
    try {
      const result = assuntoFormSchema.safeParse(form);
      if (result.success) {
        setNotification(notificationInitialState);
        setIsLoading(true);
        const response = await fetch("/api/assuntos/new", {
          method: "POST",
          headers: {
        ...
        const { message }: NewAssuntoResponse = await response.json();
    ...
    <>
      ...
      <div className="m-auto flex w-full flex-col items-center rounded-[12px] bg-light-500 text-light-50 shadow shadow-black/20 dark:bg-dark-500 dark:text-dark-50 sm:w-[25rem] md:w-[30rem] lg:w-[38rem]">
        <div className="w-full rounded-t-[12px] bg-dourado py-1 text-center">
          <h2 className="text-2xl font-light text-white">Novo Assunto</h2>
        </div>
        <form
...
AssuntoCreate.layout = "dashboard";
export default AssuntoCreate;
