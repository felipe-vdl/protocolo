...
export default function Dropdown({
  ...
  return (
    <div
      key={uuid()}
      className="z-20 border-b border-zinc-300 bg-light-900 dark:border-dark-500 dark:bg-dark-900"
    >
      <div
      ...
      >
        {links.map((link) => (
          <React.Fragment key={uuid()}>
            {link.sessionRoles ? (
              <>
                {session && link.sessionRoles.includes(session.user.role) && (
                  <Link
                    href={link.href}
                    title={link.title}
                    className={`${
                  ...
                  >
                    {link.icon}
                    {!sidebarIsCollapsed && (
                      <h3 className="hidden sm:block">{link.title}</h3>
                    )}
                  </Link>
                )}
              </>
